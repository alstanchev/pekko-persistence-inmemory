...
class EventsByPersistenceIdTest extends QueryTestSpec {
  ...
  it should "find events from an offset" in {
    ...
    withEventsByPersistenceId()("my-1", 0, 1) { tp =>
      tp.request(Long.MaxValue)
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(1), "my-1", 1, "a-1", 0L, None))
      tp.expectComplete()
    }
    ...
    withEventsByPersistenceId()("my-1", 1, 2) { tp =>
      tp.request(Long.MaxValue)
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(1), "my-1", 1, "a-1", 0L, None))
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(2), "my-1", 2, "a-2", 0L, None))
      tp.expectComplete()
    }
    ...
    withEventsByPersistenceId()("my-1", 2, 3) { tp =>
      tp.request(Long.MaxValue)
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(2), "my-1", 2, "a-2", 0L, None))
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(3), "my-1", 3, "a-3", 0L, None))
      tp.expectComplete()
    }
  ...
  it should "find events for actor with pid 'my-2'" in {
    ...
    withEventsByPersistenceId()("my-2", 0, Long.MaxValue) { tp =>
      tp.request(Long.MaxValue)
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(1), "my-2", 1, "a-1", 0L, None))
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(2), "my-2", 2, "a-2", 0L, None))
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(3), "my-2", 3, "a-3", 0L, None))
      tp.expectNoMessage(expectTime)

      persist(3, 6, "my-2")

      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(4), "my-2", 4, "a-4", 0L, None))
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(5), "my-2", 5, "a-5", 0L, None))
      tp.expectNext(ExpectNextTimeout, EventEnvelope(Sequence(6), "my-2", 6, "a-6", 0L, None))
      tp.expectNoMessage(expectTime)
